...
async function run() {
  try {
    ...
    const addUserCollection = client.db("FurnitureShop").collection("users");
    const addProductsCollection = client.db("FurnitureShop").collection("products");
    const addCardsCollection = client.db("FurnitureShop").collection("cards");
    const paymentCollection = client.db("FurnitureShop").collection("payments");
    const cardAddCollection = client.db("FurnitureShop").collection("cardAdd");
...
app.get("/", (req, res) => {
  res.send("Furniture shop is running");
});

app.listen(port, () => {
  console.log(`Furniture shop  is on port: ${port}`);
});
